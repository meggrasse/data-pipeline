...
`Pipeline` supports the following functionality:
    ...
    - `Message`s can be streamed from one or more sources, and will be merged together by the pipeline into a single `MessageStream` in the order they were received. Merging is achieved via Go's `WaitGroup` (i.e., a semaphore).
    - Whilst merging, the pipeline ensures the `Message`'s schema/version pair is valid (i.e., known by the pipeline).
    ...
    - Processing stages are responsible for closing `out` once they've drained their input. This matches Go idiom where producers are responsible for closing channels (the processing stage is considered the producer here as they are the only writer to the channel, even though they don't technically create it). 
...
## Limitations
    ...
    - client responsibility (passing message through, error handling, closing channels)
